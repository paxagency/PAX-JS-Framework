    ...
    form:function(data,frame,nest){
        ...
        $.each(frame,function(i,row){
            ...
            if(row[0].type=='group') {
                ...
                h+=`<label class="label">${row[0].label} <a onclick='pax.rend.add(${fr},"${row[0].id}-${id}")'>+</a></label> <div class='form-container'>`;
                //if(!data[row[0].id]) data[row[0].id] = [{}];
                $.each(data[row[0].id],function(z,drow){
                    var q = self.form(drow,row[0].frame,1);
                ...
                $.each(row,function(n,td){
                    ...
                    <label for="${td.id}-${id}" class="label" >${td.label}</label>`;
                    //h+=`<input type="text" name="${td.id}"  id="${td.id}-${id}"  value="${val}"/>`;
                    h+=self.vewType(td,data,td.id+'-'+id);
                    h+=`</div>`;
                });
            }
        ...
        if(nest) h+="<a onclick='pax.rend.remove(this)' class='remove'>x</a>";
        return h+'</ul>';
    },
    genId: function(){
    ...
    /*form2:function(data,frame){
        ...
        if(!frame){
            ...
            $.each(data,function(k,o){
                frame.push([{type:'text',label:k,id:k}]);
            });
        }

        $.each(frame,function(i,row){
            showRow = (row.length==1 && row[0].type=='hidden') ? 0 : 1;
            if(showRow) h+="<li data-key='"+i+"' class='li-row"+i+"'>";

            $.each(row,function(n,td){

                if(td.type=='group'){
                    var l = (data[td.id]) ? data[td.id].length : 1;
                    var z = 0;
                    var frame = pax.clone(td.frame);
                    while(z<l){
                        $.each(td.frame,function(x,td2){
                            
                            var dat = (data[td.id] && data[td.id][z]) ? pax.clone(data[td.id][z]) : {};
    ...
    vewType:function(td,e,name){
       ...
       // if(td.type=='body' || td.type=='paragraph') td.class.push('redactor');
        ...
        switch(td.type){
            ...
            case 'id':
                return "<input type='text' data-type='"+td.type+"' id='"+name+"' name='"+td.id+"' value=\""+val+"\"' "+attr+" "+cl+" vkey='id'>";
                case 'number':
            ...
            case 'raw':
                return val;
            case 'document':
                return "<input data-type='"+td.type+"' type='file' id='"+name+"' name='"+td.id+"' value='"+val+"' "+attr+" "+cl+">";
            ...
            case 'multi':
                ...
                h+='<select id="'+name+'" data-ran="'+ran+'"  name="'+td.id+'" '+attr+' '+cl+' multiple>';
                if(td.json){
               ...
               if(!td.options){
                ...
                return h+'</select>';
            case 'multiBasic':
                h+='<select id="'+name+'" data-type="'+td.type+'"  data-ran="'+ran+'"  name="'+td.id+'" '+attr+' '+cl+' multiple>';
                $.each(val,function(n,o){
                    h+='<option value="'+o+'" selected>'+o+'</option>';
                });
                return h+'</select>';
